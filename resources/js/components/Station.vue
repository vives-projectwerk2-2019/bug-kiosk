<template>
  <div class="station">
    ...
    <a
      v-bind:class="piClass"
      v-on:click="sendIDPi(name)"
      href
...
export default {
  ...
  mqtt: {
    "kiosk/+/ping"(data, topic) {
      console.log(
        topic.substring(topic.indexOf("/") + 1, topic.indexOf("/ping"))
      );
      if (
        this.name ==
        topic.substring(topic.indexOf("/") + 1, topic.indexOf("/ping"))
    ...
    "kiosk/+/status"(data, topic) {
      if (
        this.name ==
        topic.substring(topic.indexOf("/") + 1, topic.indexOf("/status"))
      ) {
  ...
  methods: {
    sendIDPi(pi_id) {
      this.$mqtt.publish(
        "kiosk/" + pi_id + "/program-dongle",
        '{"id":"' + this.programHashPi + '"}'
      );
    },
    clearTimer() {
      if (this.timer) {
